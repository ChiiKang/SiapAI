// Pure logic for POST /events: payload validation and the notification
// decision. No I/O here so it is unit-testable without a database
// (see logic_test.ts).
...
// The transition rule (contract): notify only when the stored status was
// RUNNING and the incoming status is READY. A new session going straight to
// READY, a repeated READY, or any transition to RUNNING never notifies.
